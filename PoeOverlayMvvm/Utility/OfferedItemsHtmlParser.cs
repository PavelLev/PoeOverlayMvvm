...
namespace PoeOverlayMvvm.Utility
{
    public static class OfferedItemsHtmlParser
    {
        public static async Task<List<OfferedItem>> Parse(string data)
        {
            ...
            return document.QuerySelectorAll(".item").Select(node => {
                var offeredItem = new OfferedItem();
                    
                offeredItem.Id = node.ClassList[1];
                offeredItem.Id = offeredItem.Id.Substring(offeredItem.Id.LastIndexOf("-"));
                ...
                offeredItem.Quality = node.Attributes["data-quality"]?.Value ?? node.QuerySelector("td[data-name='q']")
                                            .TextContent;
